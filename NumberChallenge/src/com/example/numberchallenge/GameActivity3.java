	        ...
	        ed_txt.addTextChangedListener(new TextWatcher() {
	            public void afterTextChanged(Editable s) {
	                if(ed_txt.getText().toString().trim().length() > 0){
	                    if(Integer.parseInt(ed_txt.getText().toString().trim()) < 14 && Integer.parseInt(ed_txt.getText().toString().trim()) > 12){
	                    	Intent intent=new Intent(getApplicationContext(), NextLevel3.class);
	                    	startActivity(intent);
