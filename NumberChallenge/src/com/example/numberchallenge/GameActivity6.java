...
public class GameActivity6 extends Activity {
	@Override
	public boolean onKeyDown(int keyCode, KeyEvent event) {
	  ...
	  private EditText ed_txt;
	    ...
	    public void onCreate(Bundle savedInstanceState) {
	        ...
	        setContentView(R.layout.activity_game_activity6);
	        ...
	        ed_txt.addTextChangedListener(new TextWatcher() {
	            public void afterTextChanged(Editable s) {
	                if(ed_txt.getText().toString().trim().length() > 0){
	                    if(Integer.parseInt(ed_txt.getText().toString().trim()) < 430 && Integer.parseInt(ed_txt.getText().toString().trim()) > 428){
	                    	Intent intent=new Intent(getApplicationContext(), NextLevel6.class);
	                    	startActivity(intent);
